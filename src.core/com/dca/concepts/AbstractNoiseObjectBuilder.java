...
/**
 * A noise object builder which uses canonical names of classes as key. Furthermore,
 * new {@link NoiseObject} classes are instantiated automatically by this
 * noise object builder.
 */
public abstract class AbstractNoiseObjectBuilder implements NoiseObjectBuilder{

	private final Map<String, Class<? extends NoiseObject>> map;

	/**
	 * Creates a new robotics builder by telling all supported classes. All classes
	 * must implement the {@link NoiseObject} interface.
	 */
	public AbstractNoiseObjectBuilder(Class<?>... types) {
		map = new HashMap<String, Class<? extends NoiseObject>>();

		if (types != null) {
			for (Class<?> type : types) {
				Class<? extends NoiseObject> roType = checkType(type);
	...
	private Class<? extends NoiseObject> checkType(Class<?> type) {
		...
		if (!NoiseObject.class.isAssignableFrom(type)) {
			throw new TypeNotSupportedException(type, "Given class is not a noise object.");
		}
		return (Class<? extends NoiseObject>) type;
	}

	@Override
	public final NoiseObject build(String name) {
	...
	/**
	 * Creates a new instance of the specified {@link NoiseObject} class.
	 *
	 * @param type the type of the instance to be created
	 * @return a new instance of the specified class
	 * @throws InstantiationFailedException if the {@link NoiseObject} cannot be
	 *                                      instantiated.
	 */
	public final NoiseObject build(Class<? extends NoiseObject> type) {
		try {
	 ...
	 */
	public final boolean canBuild(Class<? extends NoiseObject> type) {
